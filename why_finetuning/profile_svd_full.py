...
import math
from kernel.flash_attn_triton import flash_attn_triton
...
def main():
    ...
    print(f"LowRank SVD w/ mask   | {metric_name}={score:.4f} | peak={peak_m:6.1f} MiB | transient={peak_m - CACHED_ORIG_MEM:6.1f} MiB | {latency:6.1f} ms/b")
    
    # Test without mask  
    score, peak_nm, latency = acc_peak_time(model, use_mask=False)
    print(f"LowRank SVD w/o mask  | {metric_name}={score:.4f} | peak={peak_nm:6.1f} MiB | transient={peak_nm - CACHED_ORIG_MEM:6.1f} MiB | {latency:6.1f} ms/b")
