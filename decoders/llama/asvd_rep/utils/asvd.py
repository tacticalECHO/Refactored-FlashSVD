...
def main(args):
    ...
    # After quantization, before evaluation
    # Compose a unique subfolder name based on key arguments
    save_keys = [
        f"model-{args.model_id.replace('/', '-')}",
        f"actaware-{int(args.act_aware)}",
        f"alpha-{args.alpha}",
        f"ncalib-{args.n_calib_samples}",
        f"calib-{args.calib_dataset}",
        f"scale-{args.scaling_method}",
        f"sens-{args.sensitivity_metric}",
        f"wq-{args.weight_quant}",
        f"sigma-{args.sigma_fuse}",
        f"kv-{int(args.compress_kv_cache)}",
        f"kvrt-{args.kv_cache_ratio_target}",
        f"rankalign-{args.rank_align}",
        f"seed-{args.seed}"
    ]
    svd_dir = os.path.join("svd_model", "_".join(save_keys))
    if not os.path.exists(svd_dir):
        os.makedirs(svd_dir)
    print(f"Saving SVD-compressed model to {svd_dir}")
    model.save_pretrained(svd_dir)
    tokenizer.save_pretrained(svd_dir)
    print(f"SVD-compressed model saved at: {os.path.abspath(svd_dir)}")
