...
def _demo_attn_kernel(
    ...
    # per-query padding mask from [B,1,1,N] (broadcast over heads/queries)
    pad_q_ptrs = (
        mask_ptr
      + off_b * sMb
      + off_h * sMh
    ...
    for start_n in range(0, seqlen, BLOCK_M):
        ...
        # QK^T → apply mask (padding + causal) → online softmax
        qk = tl.dot(q, tl.trans(k)) * softmax_scale
        neg_inf = tl.full(qk.shape, float("-inf"), dtype=tl.float32)
        causal = (offs_m[:, None] >= (start_n + offs_n)[None, :])
        key_mask = mask_vals[None, :] & causal
        qk = tl.where(key_mask, qk, neg_inf)
    ...
    out = acc / den
    # zero-out padded queries again (safety)
    out = out * pad_q[:, None]
