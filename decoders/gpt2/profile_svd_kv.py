...
# -*- coding: utf-8 -*-
"""
profile_svd_kv.py — GPT‑2 SVD blocks with dense KV cache (evaluation only)

- Per-head SVD for Q/K/V (+ optional low-rank O/FF inside the block).
- Dense KV cache: store dense K,V per layer and grow along T.
- Evaluation:
  • Prefill perplexity on WikiText-2 (no cache).
  • Decode-style perplexity using our internal dense KV cache.
"""
...
# =========================
# SVD cache (low-rank factors)
# =========================
class DenseKVCache:
    """
    Holds per-layer dense K,V with shape [B,H,T,dh].
    """
    ...
    def update(self, K_new: torch.Tensor, V_new: torch.Tensor, layer_idx: int):
        """Append new [B,H,S_new,dh] along time dim=2."""
        ...
        if entry is None:
            ...
            self.layers[layer_idx] = (
                torch.cat([Pk, K_new], dim=2),
                torch.cat([Pv, V_new], dim=2),
            )


# =========================
# Low-rank GPT-2 Block (SVD-capable)
# =========================
...
def _attach_dense_cache_to_shims(model, dense_cache: DenseKVCache):
    """Attach shared DenseKVCache to all shims in the stack."""
...
class LayerShim(nn.Module):
    """
    Dense-KV shim: uses an injected DenseKVCache shared across layers.
    """
    ...
    def forward(self, hidden_states, past_key_value=None, cache_position=None, attention_mask=None, *args, **kwargs):
        ...
        if isinstance(dense_cache, DenseKVCache):
            ...
            if entry is not None and dense_cache.get_seq_length(self.layer_idx) > 0:
                layer_past = entry  # (K_past, V_past)
...
@torch.no_grad()
def estimate_kv_bytes_asvd(cache):
...
@torch.no_grad()
def measure_kv_bytes_asvd(cache) -> int:
    """
...
def perplexity_decode_cached(
    ...
    *,
    ...
    mdl.eval()
    total_loss, total_tokens = 0.0, 0
    # use internal DenseKVCache (not HF DynamicCache)

    if torch.cuda.is_available():
        torch.cuda.synchronize(); torch.cuda.reset_peak_memory_stats()
    t0 = time.perf_counter()

    # Track Dense KV footprint across batches (average)
    kv_bytes_total = 0
    kv_batches = 0
    ...
    for b_idx, batch in enumerate(loader):
        ...
        for t in range(0, S - 1):
            ...
            _attach_dense_cache_to_shims(mdl, dense_cache)
        ...
        # measure Dense KV size accumulated this batch
        if isinstance(dense_cache, DenseKVCache):
            kv_bytes_total += float(measure_kv_bytes_asvd(dense_cache))
            kv_batches += 1
...
def main():
    ...
    # SVD/ASVD model
    print("\n=== Building SVD Model (Dense KV cache) ===")
    ...
    print(f"SVD model built (Dense KV) with per-head rank≈{args.rank_ratio_attn}*min(D,dh) and MLP ranks≈{args.rank_ratio_mlp}*...")
    ...
    if args.mode in ("decode", "both"):
        print("\n=== Decode Perplexity (Dense KV cache) ===")
        ppl_svd_dec, kv_mib_avg, peak_mib, t_svd_dec = perplexity_decode_cached(svd_model, loader, device)
        print(f"Decode (DenseKV) | ppl={ppl_svd_dec:.4f} | KV≈{kv_mib_avg:7.1f} MiB | peak={peak_mib:7.1f} MiB | {t_svd_dec:6.1f} ms/b")
...
# Example
# python3 profile_svd_kv.py --mode both --batch-size 1 --max-length 512 --max-eval-samples 8 --rank-ratio-attn 1.0 --rank-ratio-mlp 1.0
